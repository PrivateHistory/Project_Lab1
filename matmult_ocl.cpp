...
//define typedef union to convert a Matrix to a float array
typedef union {
    Matrix<float> *matrix;
    float *array;
} MatrixUnion;
    ...
    if ((err != CL_DEVICE_NOT_FOUND) || (num_devices != 0)) {
        ...
        for (auto platform_type_device : platform_type_devices) {
            ...
            for (unsigned int i = 0; i < info_queries.size(); i++) {
                ...
                switch (info_types[i]) {
                    case ClInfoType::SIZE_T:
                        //std::cout << *reinterpret_cast<size_t *>(query) << std::endl;
                        break;
...
Matrix<float> multiplyMatricesOCL(Matrix<float> a,Matrix<float> b){
    ...
    CHECK(clEnqueueWriteBuffer(queue,aBuffer,CL_TRUE,0,a.rows*a.columns*sizeof(float),a.values.get(),0,nullptr,nullptr));
    CHECK(clEnqueueWriteBuffer(queue,bBuffer,CL_TRUE,0,b.rows*b.columns*sizeof(float),b.values.get(),0,nullptr,nullptr));
...
Matrix<double> multiplyMatricesOCL(Matrix<double> a,Matrix<double> b) {
        ...
        CHECK(clEnqueueWriteBuffer(queue,aBuffer,CL_TRUE,0,a.rows*a.columns*sizeof(double),a.values.get(),0,nullptr,nullptr));
        CHECK(clEnqueueWriteBuffer(queue,bBuffer,CL_TRUE,0,b.rows*b.columns*sizeof(double),b.values.get(),0,nullptr,nullptr));
        //Set the kernel arguments
        CHECK(clSetKernelArg(kernel,0,sizeof(cl_mem),&aBuffer));
        CHECK(clSetKernelArg(kernel,1,sizeof(cl_mem),&bBuffer));
        CHECK(clSetKernelArg(kernel,2,sizeof(cl_mem),&cBuffer));
